...
let tableCadenasAd;
let ttableDetallesCadenasAv;
let rowTable = "";
let divLoading = document.querySelector('#divLoading');
document.addEventListener('DOMContentLoaded', function(){

    tableCadenasAd = $('#tableCadenasAd').dataTable( {
        "aProcessing":true,
        "aServerSide":true,
        "language": {
        ...
        "ajax":{
            "url": " "+base_url+"/Cadenas/getCadenasAd",
            "dataSrc":""
        },
        "columns":[
            {"data":"idcadena"},
...
function fntViewCadenaAd(idcadena){
...
function fntEditCadenaAd(idcadena){
    window.location.href = base_url+"/Cadenas/EditarCadAd/"+idcadena;
}

function fntDelCadenaAd(idcadena){

    swal({
        title: "Eliminar Cadena de Adoracion",
            ...
            request.onreadystatechange = function(){
                if(request.readyState == 4 && request.status == 200){
                    ...
                    if(objData.status)
                    {
                        ...
                        tableCadenasAd.api().ajax.reload();
                    }else{
