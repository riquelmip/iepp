...
document.addEventListener('DOMContentLoaded', function(){
    ...
    tableAlabanzas = $('#tableAlabanzas').dataTable( {
        "aProcessing":true,
        "aServerSide":true,
        "language": {
        ...
        "ajax":{
            "url": " "+base_url+"/Alabanzas/getAlabanzas",
            "dataSrc":""
        },
        "columns":[
            {"data":"id"},
    ...
    //NUEVO USUARIO
    if (document.querySelector("#formAlabanzas")) {
        
    

    let formAlabanzas = document.querySelector("#formAlabanzas");
        formAlabanzas.onsubmit = function(e) {
            e.preventDefault();
           ...
           tinyMCE.triggerSave();//guardar lo que hay en el editor guardarlo en el txtalabanza
            ...
            let ajaxUrl = base_url+'/Alabanzas/setAlabanzas'; 
            let formData = new FormData(formAlabanzas);
                ...
                if(request.readyState == 4 && request.status == 200){
                    ...
                    if(objData.status)
                    {
                        ...
                        if (rowTable == "") {
                            ...
                            tableAlabanzas.api().ajax.reload();
                        }else{
...
function openModal()
{
    ...
    document.querySelector("#formAlabanzas").reset();
    $('#modalFormAlabanzas').modal('show');
}
function fntViewAlabanzasC(idalabanza){
    //let idpersona = idpersona;
    let request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
    let ajaxUrl = base_url+'/Alabanzas/getAlabanza/'+idalabanza;
    ...
    request.onreadystatechange = function(){
        if(request.readyState == 4 && request.status == 200){
            ...
            if(objData.status)
            {
                ...
                document.querySelector("#modalViewBodyC").innerHTML = "<div class='text-center'>"+objData.data.alabanza+"</div>";
                
                $('#modalViewAlabanzas').modal('show');
            }else{
                swal("Error", objData.msg , "error");
            }
        }
...
function fntEditAlabanzasC(element, idalabanza){
    ...
    request.onreadystatechange = function(){
        ...
        if(request.readyState == 4 && request.status == 200){
            ...
            if(objData.status)
            {
                document.querySelector("#idAlabanza").value = objData.data.id;
...
function fntDelAlabanzasC(idalabanza){
    ...
    swal({
        ...
        {
            let request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
            let ajaxUrl = base_url+'/Alabanzas/delAlabanza';
